...
!!! tip
    We'll be getting into the AG2 code-base, it's useful to understand how AG2 works under the hood, [see this section](../how-ag2-works/overview.md) for the rundown.
...
Let's take a look at a tool-based agent, the `DiscordAgent`. You can read more about how it's used [here](../../user-guide/reference-agents/communication-platforms/discordagent.md).
...
Let's look at one of the tools that the agent is using, [DiscordSendTool](../../user-guide/reference-tools/communication-platforms/discord.md), to see how a `Tool` is created (annotations added):

(See the [Creating a Tool](creating-a-tool.md) page for a more detailed guide on creating tools)
...
An important aspect of this tool is the use of AG2's dependency injection functionality. This protects the bot_token, guild_name, and channel_name, from the tool definition for the LLMs, so the LLM will not see the attributes or values. See [Tools with Secrets](../../user-guide/advanced-concepts/tools/tools-with-secrets.md) for guidance.
...
!!! tip
    It's important to understand how ConversableAgent generates a reply, see [this page](../how-ag2-works/generate-reply.md) for more details.
...
As an example, here's the [notebook](../../use-cases/notebooks/notebooks/agentchat_tools_commsplatforms.md) for the Discord, Slack, and Telegram tools.
...
See this [test file](https://github.com/ag2ai/ag2/blob/main/test/agents/experimental/websurfer/test_websurfer.py) for the `WebSurferAgent` as an example.

See this documentation for how to run [tests locally and coverage](../tests.md).
...
See our [Contributor Guide](../contributing.md) for more guidance.
