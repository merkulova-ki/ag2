...
llm_config = LLMConfig({"model": "gpt-5-nano", "api_key": os.environ["OPENAI_API_KEY"]})
...
See the DiscordSendTool example in the [Creating an Agent](creating-an-agent.md#how-a-tool-based-agent-is-created) documentation to see how it is implemented for a tool.

See [Tools with Secrets](../../user-guide/advanced-concepts/tools/tools-with-secrets.md) for guidance.
...
As an example, here's the [notebook](../../use-cases/notebooks/notebooks/agentchat_tools_crawl4ai.md) for the Crawl4AI tool.
...
See this [test file](https://github.com/ag2ai/ag2/blob/main/test/tools/experimental/browser_use/test_browser_use.py) for the `BrowserUseTool` as an example.
...
If it makes sense to have an agent pre-built with your tool(s), consider creating a [tool-based agent](creating-an-agent.md#how-a-tool-based-agent-is-created). Now, when you need your tool you can just add the agent to your workflow.
