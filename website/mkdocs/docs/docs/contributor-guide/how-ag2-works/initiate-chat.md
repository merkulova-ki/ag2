...
llm_config = LLMConfig({"model": "gpt-5-nano", "api_type": "openai", "cache_seed": None})
...
3. We evaluate any "process_message_before_send" hooks, see [Hooks](hooks.md), allowing functions to update the message before it gets sent to the recipient.

4. Append the message to our messages list.

5. Now we're on `agent-B` receiving the message (API Reference: `receive`).

6. The agent generates a reply given the message(s) received. This is a critical step in the process, see [Generating a reply](generate-reply.md) for an in-depth look at this step.

    - Evaluate our "update_agent_state", "process_last_received_message", and "process_all_messages_before_reply" hooks (see [Hooks](hooks.md)). These allow agent and message state to be updated before the following reply functions are evaluated.
...
2. The sending agent, `agent_A` generates their reply to the messages (see [Generating a reply](generate-reply.md)).
