...
This composes naturally with several remotes — give each `#!python as_tool()` a distinct `name=` and let the local LLM route by capability. See [Sub-task Delegation](../task_delegation.md) for the general `#!python as_tool()` semantics.
...
| `max_reconnects` | `#!python int` | `#!python 3` | Streaming reconnect attempts (see [Advanced](advanced.md#streaming-reconnect)) |
