...
**Classic (non-beta) primitives:** `#!python DefaultPattern`,
...
| Confidence threshold via `#!python ExpressionContextCondition("confidence < 7")` | Custom `#!python ContextThreshold` condition (recipe in [Context Variables → Custom Conditions](../context_variables.md#custom-conditions)) |
