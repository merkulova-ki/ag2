...
!!! tip
    `AssemblerMiddleware` and `AssemblyPolicy` live in `#!python autogen.beta.assembly` if you need to wire the middleware manually (for example, inside a custom harness that isn't built on `Agent`).
...
All six built-ins are importable from `#!python autogen.beta.policies`.
