...
!!! example "Try it on the AG2 Playground"
    See LLM and context condition handoffs live at [playground.ag2.ai](https://playground.ag2.ai){:target="_blank"} — no setup required.
...
- `FunctionTarget`: Transfer control to a custom function that can run logic before determining the next transition
- `AskUserTarget`: Ask the user to select the next speaker
- `NestedChatTarget`: Target that represents a nested chat configuration
...
A `FunctionTarget` allows you to hand off from an agent to a custom Python function, pausing the workflow so you can run arbitrary logic before deciding what happens next.

The function receives:

- The most recent agent output (as a string)
- The shared `ContextVariables`
- Optionally extra arguments

It can perform any custom logic, and then returns a `FunctionTargetResult` that specifies the next transition target and includes a message for the next agent.

Some common use cases for `FunctionTarget` include:

#### Complex validation and handoff behavior

It is often important to validate an agent’s output, particularly when its task is complex or has a reasonable chance of failure. While you can create validation agents, they often struggle to determine handoffs correctly and can clutter the group chat message history.
Function Targets offer a cleaner solution: you can write custom validation logic and dynamically decide how to proceed.

#### Custom Data Flow Handling

AG2 handles agent context automatically, but it is often important for agents to have access to specific information stored in context variables or elsewhere.
Function Targets allow you to fetch that information and include it explicitly as a message sent to the next agent.
...
First, let's create our context variables, tool functions, and agents:
```python
import os
import pprint
...
### Creating Tool Functions and Agents with Tools

Now let's define the tool functions and the agents that use them. We create the agents that tool functions reference first (like `tech_agent`), then define the tools, and finally create agents that need those tools registered via the `functions` constructor parameter:

```python
# Agents that don't need tool functions
...
# Agents with tool functions registered at construction time
...
The `group_after_work` parameter accepts any of the [transition targets](handoffs.md#transition-targets) available in AG2.
...
???+ info "Complete Code Example"
    ...
    # Create agents that don't need tool functions
    # General support for non-technical questions
    general_agent = ConversableAgent(
        name="general_agent",
        system_message="""You are a general support agent who handles non-technical questions.
    ...
    # Create agents with tool functions registered at construction time
    # Main triage agent - the starting point for all user queries
    triage_agent = ConversableAgent(
        name="triage_agent",
        system_message="""You are a support triage agent. Your role is to:
