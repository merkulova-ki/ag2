...
# Regex guardrail to detect inappropriate or harmful requests
inappropriate_request_guardrail = RegexGuardrail(
    name="inappropriate_request_detector",
    condition=r"(?i).*(hack|crack|break\s+into|unauthorized\s+access|phish|illegal).*",
    target=AgentTarget(escalation_agent),
    activation_message="Inappropriate request detected - routing to escalation"
)

# Regex guardrail to detect if agent responses accidentally include personal data
data_leak_guardrail = RegexGuardrail(
    name="data_leak_detector",
    condition=r"(?i).*(account\s+number|password|api[_-]?key|\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}).*",
    target=AgentTarget(compliance_agent),
...
Here's the full working example that demonstrates regex-based guardrails:
