...
!!! example "Try it on the AG2 Playground"
    See sequential chat running live at [playground.ag2.ai](https://playground.ag2.ai){:target="_blank"} — no setup required.
...
The maximum number of turns in each chat can be controlled with the `max_turns` key. Each chat can also terminate before the `max_turns`, see the [Ending a Chat](ending-a-chat.md) topic for further information.
