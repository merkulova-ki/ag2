...
Different tasks may require different models, and `LLMConfig` accepts multiple configuration dictionaries to specify different endpoints and configurations. Each dictionary contains the following keys depending on the kind of endpoint being used:
...
A common, useful pattern used is to define your model configurations via JSON (specified as a file or an environment variable set to a JSON-formatted string) and then use the `from_json` method to load it:
...
Passing multiple configuration dictionaries allows you to define multiple models that can be used by the agent. This is useful for a few reasons:
...
### How does an agent decide which model to pick?

An agent uses the very first model configuration and makes LLM calls against this model. If the model fails (e.g. API throttling) the agent will retry the request against the 2nd configuration and so on until prompt completion is received (or throws an error if none of the models successfully completes the request). In general there's no implicit/hidden logic inside agents that is used to pick "the best model for the task". However, some specialized agents may attempt to choose "the best model for the task". It is developers responsibility to pick the right models and use them with agents.

### Config filtering

As described above, the configurations can be filtered based on certain criteria. This is defined as a dictionary of key to filter on and values to filter by. For example, if you want to select a config with specific model(s), you can pass either a single string or a list of strings:
...
This can then be applied to a config with the `where` method:
...
Model names can differ between OpenAI and Azure OpenAI, so tags offer an easy way to smooth over this inconsistency. Tags are a list of strings in each configuration dictionary, for example:

```python
llm_config = autogen.LLMConfig(
    {"api_type": "openai", "model": "my-gpt-4.1-deployment", "api_key": "", "tags": ["gpt4o", "openai"]},
    {"api_type": "openai", "model": "llama-7B", "base_url": "http://127.0.0.1:8080", "tags": ["llama", "local"]},
)
```

When filtering, you can pass a single tag or multiple tags. A config is selected if it has at least one of the specified tags:

```python
# Single tag
filter_dict = {"tags": "llama"}
filtered = llm_config.where(**filter_dict)
assert len(filtered.config_list) == 1
...
Besides the model configuration dictionaries, there are other parameters that can be used to configure the LLM. These are split between parameters specifically used by Autogen and those passed into the model client.
...
See [this notebook](../../use-cases/notebooks/notebooks/agentchat_config_loader_utility_functions.md) for examples of using the above functions.
