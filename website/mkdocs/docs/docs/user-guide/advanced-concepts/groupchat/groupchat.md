...
We also initialize the group chat with an empty message list and a maximum round of 6, which means there will be at most 6 iterations of selecting speaker, agent speaks and broadcasting message.

!!! note
    **Unique Agent Names Required**: All agents in a `GroupChat` must have unique names. If you attempt to create a `GroupChat` with duplicate agent names, a `ValueError` will be raised. This ensures that speaker selection and message routing work correctly.

    ```python
    # This will raise a ValueError
    agent1 = ConversableAgent("assistant", ...)
    agent2 = ConversableAgent("assistant", ...)  # Duplicate name!
    group_chat = GroupChat(agents=[agent1, agent2], ...)  # ValueError!
    ```
