...
For general tracing setup (installation, `TracerProvider` configuration, backend integration), see the [OpenTelemetry Tracing](local-setup.md) page.
...
These are in addition to the standard agent span attributes (`gen_ai.operation.name`, `gen_ai.agent.name`, `ag2.span.type`). See the [Semantic Attributes](opentelemetry.md#semantic-attributes) section for the full list.
...
The [Local OpenTelemetry Setup](local-setup.md) page provides a Docker Compose stack with an OpenTelemetry Collector, Grafana Tempo, and Grafana. Point all your services' exporters at `http://localhost:14317` and open <a href="http://localhost:3333" className="external-link" target="_blank" rel="noopener noreferrer">http://localhost:3333</a> to explore traces.
