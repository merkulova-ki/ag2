...
- Token usage and cost correctly as per Google's API costs
...
!!! tip
    ...
    For guidance on using environment variables, see our [LLMs documentation](../basic-concepts/llm-configuration.md).
...
Enable streaming by setting `"stream": True` in your config. Use `run()` to iterate over events and process `StreamEvent` instances for real-time token output.
