...
```python
# V1 - Requires parsing nested structures
...
for message in response.messages:
    for content_block in message.content:
        if isinstance(content_block, ToolCallContent):
            print(f"Tool: {content_block.name}")
        elif isinstance(content_block, ImageContent):
            print(f"Image: {content_block.image_url}")

# V1 - Requires manual parsing
response = client.create(params)
...
For more detailed information on retry configuration, see the [Exponential Backoff and Retry Configuration notebook](../../use-cases/notebooks/notebooks/agentchat_bedrock_client_exponential_backoff_and_retry_config.md).
