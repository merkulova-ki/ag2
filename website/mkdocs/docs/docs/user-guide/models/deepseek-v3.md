...
description: "Use DeepSeek API models (deepseek-chat, deepseek-reasoner) with AG2 for multi-agent workflows. Configuration, tool calling, and code examples."
---

[DeepSeek](https://www.deepseek.com/) is a family of high-performance Mixture-of-Experts (MoE) language models. DeepSeek models rank among the top open-source models and match the performance of leading closed-source alternatives, with competitive pricing.

## Available DeepSeek Models

The table below lists DeepSeek API models (as of March 2026). See [DeepSeek API Pricing](https://api-docs.deepseek.com/quick_start/pricing) for the full and up-to-date list.

| Model ID | Version | Context | Input / Output (per 1M tokens) | Capabilities |
|----------|---------|---------|-------------------------------|--------------|
| `deepseek-chat` | DeepSeek-V3.2 | 128K | $0.28 / $0.42 | Chat, tool calling, JSON output, FIM completion |
| `deepseek-reasoner` | DeepSeek-V3.2 (thinking) | 128K | $0.28 / $0.42 | Reasoning, tool calling, JSON output |

Both models support cache hits at $0.028 per 1M input tokens.

## Getting Started

This guide demonstrates how to use DeepSeek models with AG2.
...
Here’s the basic configuration for DeepSeek:
...
DeepSeek supports standard OpenAI-compatible parameters. For the full list, see the [DeepSeek API documentation](https://api-docs.deepseek.com/api/create-chat-completion).
...
!!! tip
    It is recommended to set `temperature` or `top_p` but not both.
...
DeepSeek supports parallel tool calling — it can recommend calling multiple tools at once. Here’s a travel agent example with weather and currency tools.
...
DeepSeek recommended invoking both tools with the appropriate parameters. The user_proxy executed these calls, and the results were passed back to DeepSeek for interpretation and response.
