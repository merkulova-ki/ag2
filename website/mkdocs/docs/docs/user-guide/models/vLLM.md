...
Note: vLLM supports OpenAI-compatible [Function Calling](https://platform.openai.com/docs/guides/function-calling) (tool use) when the server is started with `--enable-auto-tool-choice` and an appropriate `--tool-call-parser`. See the [vLLM tool calling documentation](https://docs.vllm.ai/en/latest/features/tool_calling.html) for supported models and configuration.

Running this stack requires the installation of:
1. AG2 ([installation instructions](../basic-concepts/installing-ag2.md))
2. vLLM
...
```bash
vllm serve mistralai/Mistral-7B-Instruct-v0.2 --chat-template ag2mistraltemplate.jinja
```

Note: In older versions of vLLM, use `python -m vllm.entrypoints.openai.api_server` instead of `vllm serve`.
...
assistant = ConversableAgent("agent", system_message="", llm_config=local_llm_config)
