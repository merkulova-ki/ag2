...
!!! note
    Before running the below commands, make sure to set your `OPENAI_API_KEY` as an environment variable. This example uses `gpt-5-nano`, but you can replace it with any other [model](https://docs.ag2.ai/latest/docs/user-guide/models/amazon-bedrock/) supported by AG2.
...
const runtime = new CopilotRuntime({
    agents: {
        agentic_chat: new HttpAgent({ url: `${BASE_URL}/fastagency/awp` }),
    },
});
