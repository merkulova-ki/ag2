...
categories:
    - Realtime API
date: 2025-01-08
slug: RealtimeAgent-over-websocket
---
...
## **How It Works**
At its core, the `**`WebSocketAudioAdapter`**` leverages [WebSockets](https://fastapi.tiangolo.com/advanced/websockets/) to handle real-time audio streaming. This means your browser becomes the communication bridge, sending audio packets to a server where a `**`RealtimeAgent`**` agent processes them.
...
3. **Powered by Realtime Agents**:
   ...
   - The audio adapter integrates with an AI-powered `**`RealtimeAgent`**`, allowing the agent to process audio inputs and respond intelligently.
...
### Ready to Chat? 🚀
Now you can simply open **`http://localhost:5050/start-chat`** in your browser, and dive into an interactive conversation with the `**`RealtimeAgent`**`! 🎤✨
...
3. **Set Up the `WebSocketAudioAdapter`**
   The `**`WebSocketAudioAdapter`**` bridges the client’s audio stream with the `**`RealtimeAgent`**`. It streams audio data over [WebSockets](https://fastapi.tiangolo.com/advanced/websockets/) in real time, ensuring seamless communication between the browser and the agent.
...
4. **Configure the Realtime Agent**
   ...
   - **Audio Adapter**: Connects the `**`WebSocketAudioAdapter`**` for handling audio.
