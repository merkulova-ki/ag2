...
llm_config = LLMConfig.from_json(path="OAI_CONFIG_LIST", temperature=0).where(model="gpt-5")
...
The tool library we provide requires subscribing to specific APIs, please refer to the [docs](../../../user-guide/reference-agents/captainagent.md) for details. The following example does not necessarily require tool usage, so it's fine if you are subscribing to them.
...
- [document](../../../user-guide/reference-agents/captainagent.md).
