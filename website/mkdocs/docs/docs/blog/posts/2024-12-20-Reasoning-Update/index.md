...
In our [previous post](https://docs.ag2.ai/latest/docs/blog/2024/12/02/ReasoningAgent2), we introduced the ReasoningAgent, which utilized Beam Search for systematic reasoning. Today, we include MCTS (Monte Carlo Tree Search) and Language Agent Tree Search (LATS) as alternative search strategies, which present advantages in different scenarios.
...
# Configure the model
llm_config = LLMConfig(config_list={"api_type": "openai", "model": "gpt-5-nano", "api_key": os.environ.get("OPENAI_API_KEY")})
...
# Configure the model
llm_config = LLMConfig(config_list={"api_type": "openai", "model": "gpt-5-nano", "api_key": os.environ.get("OPENAI_API_KEY")})
llm_config_larger = LLMConfig(config_list={"api_type": "openai", "model": "gpt-5", "api_key": os.environ.get("OPENAI_API_KEY")})
...
* [Original ReasoningAgent with Beam Search](https://docs.ag2.ai/latest/docs/blog/2024/12/02/ReasoningAgent2)
