...
categories:
    - Tutorial
date: 2023-12-29
slug: AgentDescriptions
---

## TL;DR

AutoGen 0.2.2 introduces a `description` field to ConversableAgent (and all subclasses), and changes GroupChat so that it uses agent `description`s rather than `system_message`s when choosing which agents should speak next.
